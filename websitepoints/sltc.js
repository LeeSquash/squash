...
export function calculateSLTCPoints(records) {

    let totalPoints = 0;

    if (!records || records.length === 0) return 0;

    const rules = POINTS_CONFIGURATION.competitions.sltc.rules;
    const bonuses = POINTS_CONFIGURATION.competitions.sltc.seasonFinish;
